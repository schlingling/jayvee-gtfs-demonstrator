...
class GtfsRTValidator {
  public async countAlertsRecords(path: string): Promise<[number, number, number]> {
  ...
  public async countVehiclePositions(path: string): Promise<[number, number, number]> {
    const feed = this.initFeedMessage(path);
  ...
  public async countTripUpdates(path: string): Promise<[number, number, number]> {
  ...
  async validate() {
    console.log("------------------started------------------");
    const tripUpdateResult = await validator.countTripUpdates("resources/gtfs-rt-brest-metropole/bibus-brest-gtfs-rt-trip-update");
    const alertResult = await validator.countAlertsRecords("resources/gtfs-rt-brest-metropole/bibus-brest-gtfs-rt-alerts");
    const vehiclePositionsResult = await validator.countVehiclePositions("resources/gtfs-rt-brest-metropole/bibus-brest-gtfs-rt-vehicle-position");
    console.log("TripUpdate --> #rows in manual import: " + tripUpdateResult[0] + ", #rows in processed table:  " + tripUpdateResult[1] + " matches found: " + tripUpdateResult[2]);
    console.log("Alert --> #rows in manual import: " + alertResult[0] + ", #rows in processed table:  " + alertResult[1] + " matches found: " + alertResult[2]);
    console.log("VehiclePosition --> #rows in manual import:  " + vehiclePositionsResult[0] + ", #rows in processed table:  " + vehiclePositionsResult[1] + " matches found: " + vehiclePositionsResult[2]);
